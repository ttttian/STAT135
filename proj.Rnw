...
\item
<<echo=FALSE>>=
gender.glyhb.cond.table <- matrix(sapply(conditions,
              function(x) nrow(subset(data.df, gender.glyhb.cond==x))), nrow=2)
...
Since the $p$-value is 0.004748, which is smaller than 0.05, we reject the null hypothesis that \texttt{hdl} has equal means for those with type II diabetes and those without, with 5\% significance interval.
...
Since the $p$-value is 2.034e-06, which is smaller than 0.05, we reject the null hypothesis that \texttt{bp.1s} has equal means for those with type II diabetes and those without, with 5\% significance interval.
...
Since the $p$-value is 0.168, which is greater than 0.05, we fail to reject the null hypothesis that \texttt{bp.1d} has equal means for those with type II diabetes and those without, with 5\% significance interval.
...
Since the $p$-value is 0.001983, which is smaller than 0.05, we reject the null hypothesis that \texttt{BMI} has equal means for those with type II diabetes and those without, with 5\% significance interval.
...
Since the $p$-value is 9.95e-05, which is smaller than 0.05, we reject the null hypothesis that \texttt{WHR} has equal means for those with type II diabetes and those without, with 5\% significance interval.
...
\item
From 3.2, we know that \texttt{WHR} has a normal distribution. We assume all patients come from the same population, so the standard deviation is constant.
