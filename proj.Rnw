...
\item
\texttt{stab.glu}: The two distributions have substantial difference, so it may BE a relevant feature.
\item
...
\item
\texttt{age}: The two distributions have substantial difference, so it may BE a relevant feature.
\item
...
\begin{itemize}
\item
On average, emales have higher BMI than males.
\item
On average, eople with type II diabetes (\texttt{glyhb >= 7}) have higher BMI than people without type II diabetes (\texttt{glyhb < 7}).
\item
