...
glyhb.lm <- lm(glyhb ~ stab.glu + age + BMI, data.df)
glyhb.lm.fit <- predict(glyhb.lm, data.df)
...
err.rate <- lm.err.rate(data.df$glyhb, glyhb.lm.fit, lambda=7)
@
...
false.pos.rate <- lm.false.pos.rate(data.df$glyhb, glyhb.lm.fit, lambda=7)
false.neg.rate <- lm.false.neg.rate(data.df$glyhb, glyhb.lm.fit, lambda=7)
@
...
<<Q5.3.1>>=
################################################################################
coefficients(lm(scale(glyhb) ~ scale(stab.glu) +
                  scale(age) + scale(BMI), data.df))
################################################################################
@
...
\item
\noindent
<<Q5.5.1, cache=TRUE, echo=FALSE, fig.align="center", fig.pos="H", fig.width=5, fig.height=2.5>>=
glyhb.lm <- lm(glyhb ~ stab.glu + BMI + WHR, data.df)
glyhb.lm.fit <- predict(glyhb.lm, data.df)

ggplot(data.df) +
  geom_point(aes(x=glyhb.lm.fit, y=scale(glyhb-glyhb.lm.fit))) +
  labs(title="Residuals plot of glyhb", x="predicted values", y="standardized residuals") +
  theme(text=element_text(size=8.5))
@

\item
<<Q5.6.1, cache=TRUE, echo=FALSE>>=
glyhb.glm <- glm(as.numeric(glyhb.cond)-1 ~ scale(stab.glu) +
                   scale(age) + scale(BMI), data.df, family=binomial)
...
glm.test.err.rate <- glm.err.rate(test.df$glyhb.cond, glyhb.test.glm.fit, lambda=0.5)
@
...
We obtain better performance than in the training set for both predictors.
\end{enumerate}
