...
\item
From part 3.3, we know that \texttt{WHR} has a normal distribution. We assume all patients come from the same population, so the standard deviation is constant.
