...
glyhb.df <- data.frame(data.df, glyhb.cond=NA)
glyhb.df[glyhb.df$glyhb>=7, ]$glyhb.cond <- "glyhb >= 7"
glyhb.df[glyhb.df$glyhb<7, ]$glyhb.cond <- "glyhb < 7"
...
for (i in 1:length(features)) {
  feature <- features[i]
  feature.boxplot <- ggplot(data.frame(glyhb.cond=glyhb.df$glyhb.cond,
                                       feature=glyhb.df[, feature])) +
